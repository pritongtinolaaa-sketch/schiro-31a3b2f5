...
async function listCatchmailMessageIds(address: string): Promise<string[]> {
  ...
  return rows.map((row: any) => String(row?.id ?? "")).filter((id) => id.length > 0);
}

async function clearCatchmailInbox(address: string) {
  const ids = await listCatchmailMessageIds(address);
